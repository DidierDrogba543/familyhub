...
export default function CalendarView() {
    ...
    async function load() {
      ...
      for (const child of childrenRes.data ?? []) {
        const { data: acts } = await supabase
          ...
          .select("activity_name, day_of_week, time_slot, notes, term, link_url, link_label, kit_items")
  ...
  const getDaySchedule = (dayName: string) => {
    const morning: { child: string; activity: string; time: string; notes: string | null; link_url: string | null; link_label: string | null; kit_items: string[] }[] = [];
    const afterSchool: { child: string; activity: string; time: string; notes: string | null; link_url: string | null; link_label: string | null; kit_items: string[] }[] = [];
    const evening: { child: string; activity: string; time: string; notes: string | null; link_url: string | null; link_label: string | null; kit_items: string[] }[] = [];

    for (const child of children) {
      const dayActs = child.activities.filter((a) => a.day_of_week === dayName);
      for (const act of dayActs) {
        const hour = getStartHour(act.time_slot);
        const entry = { child: child.name, activity: act.activity_name, time: formatTime(act.time_slot), notes: act.notes, link_url: act.link_url, link_label: act.link_label, kit_items: act.kit_items || [] };
                          ...
                          {schedule.morning.map((entry, j) => {
                            ...
                            return (
                              <div key={j} className={`${colors.bg} rounded-lg px-3 py-1.5`}>
                                ...
                                {entry.kit_items.length > 0 && <span className="text-[10px] text-amber-500 ml-1" title={entry.kit_items.join(", ")}>🎒{entry.kit_items.length}</span>}
                              </div>
                            );
                          })}
                        </div>
